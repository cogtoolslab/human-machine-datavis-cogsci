...
def create_question_prompt(question, agentResponse, choices=""):
	...
	# return f"{task_description}\n{hint_multiple_choice}\n{question}\n{choices}{model_responses}\n{end}"
...
if __name__ == "__main__":
	dataset = "ggr"
	textInput = "open_ended_question"
	df = pd.read_csv(f"/Users/arnav/Desktop/contextvis/vlm-datavis-benchmark/analysis/processed_agent_responses/{dataset}_response_df_jan20.csv")
	answers_df = pd.DataFrame(columns=list(df.columns) + ["answer"])
	if dataset == "ggr" or dataset == "vlat":
	...
	for i, response in df.iterrows():
		if dataset == "ggr":
			...
			textInput = row["textInputType"]

		if response["promptType"] == "instructions_question":
			continue
		if response["agentType"] == "Salesforce/blip2-opt-2.7b":
			continue
	...
	answers_df.to_csv(f"/Users/arnav/Desktop/contextvis/vlm-datavis-benchmark/analysis/processed_agent_responses/{dataset}_answers_df_jan21.csv")
